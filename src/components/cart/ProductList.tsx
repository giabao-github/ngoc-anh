...
const ProductList: React.FC<ProductListProps> = ({ cartItems, isMobile }) => {
  ...
  const fallbackHeight =
    estimatedItemHeight * maxItems + 24 * (maxItems - 1) + 32;
  ...
  const updateMaxHeight = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const calculatedHeight =
        totalHeight + spacing + 2 * containerPadding + scrollAreaBuffer;
  ...
  return (
    <ScrollArea ref={scrollAreaRef} className={isMobile ? "px-3" : "px-8"}>
      <div
        ref={containerRef}
      ...
      >
        {cartItems.map((item, index) => {
          ...
          return (
            <div data-item-index={index} key={item.slug}>
              <CartProduct item={item} index={index} product={product} />
            </div>
          );
        })}
      </div>
