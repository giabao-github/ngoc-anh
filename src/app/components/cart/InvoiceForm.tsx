...
import TaxCodeInput from "@/app/components/cart/TaxCodeInput";
import EmailInput from "@/app/components/user/EmailInput";
import { sanitizeInput, sanitizeInputOnBlur } from "@/app/lib/utils";
...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ contentRef, contentHeight }) => {
    ...
    >
      <div ref={contentRef} className="flex flex-col gap-y-2 font-medium">
        <Input
          ...
          onBlur={(e) => handleChange("companyName", sanitizeInputOnBlur(e.target.value))}
        ...
        <Input
          ...
          onBlur={(e) => handleChange("address", sanitizeInputOnBlur(e.target.value))}
          className={`w-full text-sm md:text-base bg-neutral-100 border border-gray-300 focus:border-black rounded md:tracking-wide ${montserrat.className}`}
        />
        <button
          onClick={saveForm}
