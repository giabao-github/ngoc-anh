...
import { createCartItem, getCartFromStorage } from "@/app/lib/productUtils";
...
export const useAddToCart = (
  ...
  const isOutOfStock = useMemo(() => availableQuantity === 0, [availableQuantity]);
  ...
  const addToCart = useCallback((quantity: number) => {
    ...
    try {
      ...
      const existingIndex = existingCart.findIndex(
        (item: CartItem) => item.id === product.id && item.name === product.name
      );
  ...
  }, [product, availableQuantity, cartQuantity, isMobile, updateCartCount, imageRef, cartIconRef, showNotificationWithTimeout]);
