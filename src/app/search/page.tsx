        ...
        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
          {results.map((product: Product) => (
              <div key={product.id} className="bg-neutral-100 border border-neutral-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200">
                ...
                <h2
                  onClick={() => router.push(`/products/${product.patterns[0].slug}`)}
                  className="px-3 py-2 font-semibold text-xl text-[#0C2543] cursor-pointer hover:underline"
                >
                  {product.name}
                </h2>
                <p className="px-3 pb-3 text-lg font-semibold mt-2">{product.patterns[0].price.toLocaleString()}₫</p>
              </div>
            ))
          }
        </div>
